...
debug_break;
		...
		//////
			switch (tnIn)
			{
				case _CONVERSION_FUNCTION_TIME:
					// Hh:Mm:Ss[.Mss]
					if (!iVariable_isTypeCharacter(varP1) || varP1->value.length < 8 || varP1->value.data_s8[2] != ':' || varP1->value.data_s8[5] != ':')
					{
						iError_report_byNumber(_ERROR_INVALID_ARGUMENT_TYPE_COUNT, iVariable_get_relatedComp(varP1), false);
						return;
					}
				...
				case _CONVERSION_FUNCTION_TIMEX:
					...
					// Hh:Mm:Ss[.Micsss]
					// Hh:Mm:Ss[.Nanosssss]
					if (!iVariable_isTypeCharacter(varP1) || varP1->value.length < 12 || varP1->value.data_s8[2] != ':' || varP1->value.data_s8[5] != ':')
					{
						iError_report_byNumber(_ERROR_INVALID_ARGUMENT_TYPE_COUNT, iVariable_get_relatedComp(varP1), false);
						return;
					}
	...
	void ifunction_type_common(SReturnsParams* rpar, SVariable* var, bool tlExtraInfo, bool tlIsVartype, bool tlNullIsType)
	{
		...
		//////
			...
			if (tlExtraInfo)
			{
				...
				// Standard
				if ((!var->value._data || var->value.length == 0) && tlIsVartype)
				{
					// NULL
					c = 'X';
...
// 								case 'V':	// Varchar
		...
		//////
			if (tlIsVartype && !tlNullIsType && (var->varType == _VAR_TYPE_NULL || !var->value._data || var->value.length == 0))
				c = 'X';	// It is NULL
