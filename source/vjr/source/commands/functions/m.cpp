			...
			switch (varLeft->varType)
			{
				...
				case _VAR_TYPE_CHARACTER:
					if (varLeft->value.length == 0 || varRight->value.length == 0)
					{
						...
						switch (_memicmp(varLeft->value.data_s8, varRight->value.data_s8, min(varLeft->value.length, varRight->value.length)))
						{
							case -1:
							case 0:
	...
	void function_min(SReturnsParams* rpar)
	{
		...
		//////
			...
			switch (varLeft->varType)
			{
				...
				// Note: _VAR_TYPE_DATETIMEX is handled below as a 64-bit number
				case _VAR_TYPE_DATETIME:
					dtLeft	= (SDateTime*)varLeft->value.data_s8;
					dtRight	= (SDateTime*)varRight->value.data_s8;
					if (dtLeft->julian < dtRight->julian)
					{
						// Left is less
						llLeft = true;
