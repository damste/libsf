...
//////
	...
	{
		u32				lnI;
		SBuilder*		eiDst;		// *eiDstRoot
		SExtraInfo*		ei;			// Iterative pointer
		SExtraInfo*		eiDup;		// Orphan duplicated item
		SExtraInfo*		eiNew;		// Newly allocated item in lineDup->extra_info


		// Make sure our environment is sane
		if (eiDstRoot && eiSrc && eiSrc->populatedLength > 0)
		{
			// Do we have a target?
			if (!*eiDstRoot)
				iBuilder_createAndInitialize(eiDstRoot);

			// Grab our common pointer
			eiDst = *eiDstRoot;

			// Iterate through every extra info and copy all that can be copied
			iterate(lnI, eiSrc, ei, SExtraInfo)
			//

				// Can this line be copied?
				if (ei->_onDup)
				{
					// Duplicate
					eiDup = ei->onDup(ei);
					...
					{
						// Append the entry
						eiNew = iExtraInfo_allocate(eiDstRoot);
						if (eiNew)
						{
							// Copy from the orphan duplicated item to the actual permanent entry in lineDup
							memcpy(eiNew, eiDup, sizeof(SExtraInfo));
						}
					}
				}
