...
#if BX_REMOTE_MDA == 1
    #define BX_MDA_BOCHS1               0x12345678
    #define BX_MDA_BOCHS2               0x87654321
    #define BX_MDA_GET_MONO_MOUSE_XY    0
    #define BX_MDA_GET_MONO_CURSOR      1
	#define BX_MDA_GET_KEYSTROKE		2
    extern long bx_mda_mouse_x;
    extern long bx_mda_mouse_y;
    extern long bx_mda_mouse_buttons;
	extern long bx_mda_key;
	extern long bx_mda_key_up_down;
#endif
void BX_CPU_C::interrupt(Bit8u vector, bx_bool is_INT, bx_bool is_error_code, Bit16u error_code)
{
    ...
    if (vector == 0xff)
    {
        if (BX_CPU(which_cpu)->get_reg32(BX_32BIT_REG_EAX) == BX_MDA_BOCHS1 && 
            BX_CPU(which_cpu)->get_reg32(BX_32BIT_REG_EBX) == BX_MDA_BOCHS2)
        {
            // Interface between monochrome monitor MoMO and operating system/Bochs through software
            if (BX_CPU(which_cpu)->get_reg32(BX_32BIT_REG_ECX) == BX_MDA_GET_MONO_MOUSE_XY)
            {
                // Return the current mouse X, Y and button values
                BX_CPU(which_cpu)->set_reg32(BX_32BIT_REG_EAX, bx_mda_mouse_x);
                BX_CPU(which_cpu)->set_reg32(BX_32BIT_REG_EBX, bx_mda_mouse_y);
                BX_CPU(which_cpu)->set_reg32(BX_32BIT_REG_ECX, bx_mda_mouse_buttons);
                return;
            }
			else if (BX_CPU(which_cpu)->get_reg32(BX_32BIT_REG_ECX) == BX_MDA_GET_KEYSTROKE)
			{
				// EAX - Store key code
				// EBX - Store up/down code
				BX_CPU(which_cpu)->set_reg32(BX_32BIT_REG_EAX, bx_mda_key);
				BX_CPU(which_cpu)->set_reg32(BX_32BIT_REG_EBX, bx_mda_key_up_down);

				// Set the ZERO? flag indicating a key is ready (zero), or not (1)
				BX_CPU(which_cpu)->set_ZF((bx_bool)bx_mda_key ? 0 : 1);
				bx_mda_key			= 0;
				bx_mda_key_up_down	= 0;
				return;
			}
            else if (BX_CPU(which_cpu)->get_reg32(BX_32BIT_REG_ECX) == BX_MDA_GET_MONO_CURSOR)
            {
                // Not currently enabled
            }
            else
            {
                // Internal trap designed to throw control here to examine something
                long _eax = BX_CPU(which_cpu)->get_reg32(BX_32BIT_REG_EAX);
                long _ebx = BX_CPU(which_cpu)->get_reg32(BX_32BIT_REG_EBX);
                long _ecx = BX_CPU(which_cpu)->get_reg32(BX_32BIT_REG_ECX);
                long _edx = BX_CPU(which_cpu)->get_reg32(BX_32BIT_REG_EDX);
                long _esi = BX_CPU(which_cpu)->get_reg32(BX_32BIT_REG_ESI);
                long _edi = BX_CPU(which_cpu)->get_reg32(BX_32BIT_REG_EDI);
                long _ebp = BX_CPU(which_cpu)->get_reg32(BX_32BIT_REG_EBP);
                long _esp = BX_CPU(which_cpu)->get_reg32(BX_32BIT_REG_ESP);
                _asm int 3;
                BX_CPU(which_cpu)->set_CF((bx_bool)1);
            }
            return;
        }
    }
