...
#include "iodev/iodev.h"
    ...
    bool bx_mda_enabled = true;                     // Initially enabled, checks to see if there is a MoMo window (MOnochrome MOnitor program) running
    unsigned long bx_mda_counter = 0;               // Used with the timer
    extern unsigned char bx_mda_32kb_buffer[32768]; // Access to the external buffer (in memory.cpp)
    HWND hwndMomo = NULL;
    // These functions are in memory.cpp due to their nature relating to memory storage
	extern void CALLBACK bx_mda_named_pipe(HWND hwnd, UINT uMsg, UINT_PTR idEvent, DWORD dwTime);
    extern VOID CALLBACK bx_mda_timer_function(HWND hwnd, UINT uMsg, UINT_PTR idEvent, DWORD dwTime);
#endif
...
#ifdef BX_REMOTE_MDA
unsigned char win32_mda_32kb_buffer[32768];
#endif
...
LRESULT CALLBACK simWndProc(HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
  ...
  case WM_CREATE:
    ...
    InitFont();
#endif
    SetTimer (hwnd, 1, 333, NULL);
#if BX_REMOTE_MDA == 1
	SetTimer (hwnd, 2, 333, bx_mda_named_pipe);
    SetTimer (hwnd, 3, 333, bx_mda_timer_function);
#endif
    return 0;
